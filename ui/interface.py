...
from classes.data import create_item_info
...
class InterfaceFM(QMainWindow):
    def __init__(self):
        ...
        self._create_run_stop_button()
        self._create_item_panel()

        self.item_info = None

        self.rune_to_compute = 0
        self.number_of_stats = 0
        self.number_of_line_damage = 0
    ...
    def _create_run_stop_button(self):
    ...
    def _update_value(self):
        for i in range(self.number_of_stats):
            line_ui = self.stats[i]
            line_item = self.item_info.items[i]
            line_ui["minimum"].setText(str(line_item.minimum))
            QApplication.processEvents()
            line_ui["maximum"].setText(str(line_item.maximum))
            QApplication.processEvents()
            line_ui["effect"].setText(str(line_item.effect))
            QApplication.processEvents()
            #line_ui["modify"].setText(str(line_item.modify))
            line_ui["modify"].setText("---")
            QApplication.processEvents()
            line_ui["rune_base"].setText(str(line_item.rune_base))
            QApplication.processEvents()
            line_ui["rune_pa"].setText(str(line_item.rune_pa))
            QApplication.processEvents()
            line_ui["rune_ra"].setText(str(line_item.rune_ra))
            QApplication.processEvents()
    ...
    def run(self):
        ...
        try:
            self.number_of_line_damage = int(self.nodl_cpt.text())
        except:
            self.number_of_line_damage = 0
            self.nodl_cpt.setText("0")

        self.item_info = create_item_info(self.number_of_stats, self.number_of_line_damage)
        ...
        try:
            self.rune_to_compute = int(self.nor_cpt.text())
        except:
            self.rune_to_compute = 0
            self.rune_to_compute.setText("0")

        QApplication.processEvents()
        for i in range(self.rune_to_compute):
            if i == 0:
                self.item_info.compute_info()
                QApplication.processEvents()
                self._update_value()
                QApplication.processEvents()
            line = self.item_info.get_best_stat_to_fm()
            QApplication.processEvents()
            line.apply_rune(False, False)
            QApplication.processEvents()
            self.run_button.setText("{} / {}".format(i+1, self.rune_to_compute))
            QApplication.processEvents()
            self.item_info.compute_info()
            QApplication.processEvents()
            self._update_value()
